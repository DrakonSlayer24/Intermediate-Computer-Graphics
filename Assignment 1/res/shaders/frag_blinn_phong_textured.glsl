...
*/
		...
		//Diffuse only
		if(u_DiffuseToggle == true)
		{
			result = ((u_AmbientCol * u_AmbientStrength) + (diffuse) * attenuation) * inColor * textureColor.rgb;
		}
		//Ambient only
		else if(u_AmbientToggle == true)
		{
			result = ((u_AmbientCol * u_AmbientStrength) + (ambient) * attenuation) * inColor * textureColor.rgb;
		}
		//Specular only
		else if(u_SpecularToggle == true)
		{
			result = ((u_AmbientCol * u_AmbientStrength) + (specular) * attenuation) * inColor * textureColor.rgb;
		}
		//Diffuse + ambient
		else if(u_DiffuseToggle == true && u_AmbientToggle == true)
		{
			result = ((u_AmbientCol * u_AmbientStrength) + (ambient + diffuse) * attenuation) * inColor * textureColor.rgb;
		}
		//Diffuse + specular
		else if(u_DiffuseToggle == true && u_SpecularToggle == true)
		{
			result = ((u_AmbientCol * u_AmbientStrength) + (specular + diffuse) * attenuation) * inColor * textureColor.rgb;
		}
		//Ambient + specular
		else if(u_AmbientToggle == true && u_SpecularToggle == true)
		{
			result = ((u_AmbientCol * u_AmbientStrength) + (ambient + specular) * attenuation) * inColor * textureColor.rgb;
		}
		...
		{
			result = ((u_AmbientCol * u_CustomEffect) + (ambient + diffuse + specular) * attenuation) * inColor * textureColor.rgb;
		}
